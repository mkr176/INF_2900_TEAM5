...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['user_id', 'username', 'type', 'age', 'avatar', 'get_type_display'] # get_type_display provides human-readable type
        read_only_fields = ['username', 'user_id', 'get_type_display'] # These are derived or read-only
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'password': {'write_only': True, 'required': False}, # Password is write-only, not required for updates unless changing it
            'date_joined': {'read_only': True}, # Usually read-only
            'is_staff': {'read_only': True}, # Usually read-only unless managed by admin endpoint
        }
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    type = serializers.ChoiceField(choices=UserProfile.TYPES, required=False) # Optional, defaults to 'US' in model
    age = serializers.IntegerField(required=False, allow_null=True)
    email = serializers.EmailField(required=True) # Make email required for registration

    class Meta:
        model = User
        fields = ['username', 'email', 'password', 'password2', 'first_name', 'last_name', 'type', 'age']
        extra_kwargs = {
    ...
    def validate(self, attrs):
        ...
        attrs.pop('password2')
        # You can add more validation here if needed (e.g., check username uniqueness, although model handles it)
        return attrs
...
class BookSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'title', 'author', 'isbn', 'category', 'category_display', 'language',
            'condition', 'condition_display', 'available', 'image', 'borrower', 'borrower_id',
            'borrow_date', 'due_date', 'storage_location', 'publisher',
            'publication_year', 'copy_number', 'added_by', 'added_by_id'
        ]
        read_only_fields = [
            'borrower', 'borrower_id', 'borrow_date', 'due_date', # Usually managed by borrow/return logic
            'added_by', 'added_by_id', # Set automatically based on logged-in user
            'category_display', 'condition_display', # Read-only representations
        ]
